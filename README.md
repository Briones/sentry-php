...
Add your [Sentry DSN](https://docs.sentry.io/quickstart/#configure-the-dsn) value of your project to ``app/config/config_prod.yml`` if you have Symfony 3.4 or to `config/packages/sentry.yaml` if you have Symfony 4 or newest.
Leaving the `dsn` value empty (or undeclared) in other environments will effectively disable Sentry reporting.
